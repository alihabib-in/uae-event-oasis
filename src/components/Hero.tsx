  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentIndex((prevIndex) => 
        prevIndex === (events.length > 0 ? events.length - 1 : 2) ? 0 : prevIndex + 1
      );
    }, 4000);
              ...
              <div className="relative h-full w-full">
                {displayEvents.map((event, index) => {
                  ...
                  return (
                    <div
                      key={event.id}
                      className="absolute inset-0 w-full h-[350px] transition-all duration-700 ease-in-out cursor-pointer"
                      style={{
                        zIndex,
